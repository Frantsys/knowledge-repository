...
@Tag(
    name = "User",
    description = "API path for managing users"
)
@RestController
...
public class UserController {
    ...
    public ResponseEntity<UserResponse> updateStatus(
            @PathVariable Long id,
            @RequestBody @Valid UserUpdateActivationRequest request
    ) {

        UserResponse updatedUser =
                userService.updateActivationById(id, request);

        return ResponseEntity.ok(updatedUser);
    }

    @PatchMapping("/{id}/password")
    public ResponseEntity<UserResponse> changePasswordById(
            @PathVariable Long id,
